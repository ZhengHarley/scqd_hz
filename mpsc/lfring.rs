...
#[repr(C, align(128))]               // 128-byte cache line width
struct LFRingHeader {
...
impl LFRing {
    ...
    pub(crate) fn enqueue(&self, order: usize, eidx: usize, nonempty: bool) -> bool {
        ...
        loop {
            ...
            let tcycle = ((tail << 1) | (2 * n as u64 - 1)) as i64;             // lfatomic_t, but is stored as a signed integer?
            let tidx = map(tail, order, n);                                     // usize
            let mut entry = self.array[tidx].load(Acquire);                     // lfatomic_t

            'retry: loop {
                let ecycle = ((entry << 1) | (2 * n as u64 - 1)) as i64;        // lfatomic_t, but is stored as a signed integer?
                ...
                let can_write = lfring_cmp_lt(ecycle, tcycle) &&                // ecycle and tcycle are stored as signed for the cmp statements, change?
                    ((entry == ecycle as u64) || 
                     (entry == (ecycle as u64 ^ n as u64) && 
                      (lfring_cmp_lt(self.head.load(Acquire) as i64), tail as i64)));
    ...
    pub(crate) fn dequeue(&self, order: usize, nonempty: bool) -> usize {
        ...
        loop {
            ...
            let hcycle = ((head << 1) | (2 * n as u64 - 1)) as i64;         // why are cycles represented as signed?
            ...
            'again: loop {
                ...
                'do: loop {
                    let ecycle = ((entry << 1) | (2 * n as u64 - 1)) as i64;
                    ...
                    let entry_new = if (entry | n as u64) != hcycle as u64 {
            ...
            if !nonempty {
                ...
                if lfring_cmp_le(tail as i64, (head + 1) as i64) {
        ...
        loop {
            match self.tail.compare_exchange_weak(tail, head, AcqRel, Acquire) {
                ...
                Err(actual_tail) => {
                    ...
                    if lfring_cmp_le(tail as i64, actual_head as i64) {
